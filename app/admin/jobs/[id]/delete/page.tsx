    ...
    <div className="max-w-md mx-auto space-y-6 pt-12">
      ...
      <div className="bg-slate-900 border border-red-950/60 rounded-2xl p-6 shadow-xl text-center space-y-6">
        ...
        <div className="space-y-2">
          <h1 className="text-xl font-bold text-white">Delete Job Posting?</h1>
          <p className="text-sm text-slate-400">
            Are you sure you want to delete the job posting for <strong className="text-slate-200">{job.title}</strong> ({job.location})? 
            This opening will be permanently deleted from the website careers board.
          </p>
        </div>

        <form action={handleDelete} className="flex gap-3">
          <Link href="/admin/jobs" className="flex-1 bg-slate-855 hover:bg-slate-800 text-slate-300 text-sm font-semibold py-2.5 rounded-xl border border-slate-700 transition-all text-center block">
            Cancel
          </Link>
          <button
            type="submit"
